...
namespace GoogleARCore.Examples.CloudAnchors
{
    ...
    public class CloudAnchorsExampleController : MonoBehaviour
    {
        ...
        private int numberOfObjectsAllowed = 1;
        ...
        public void Spawn(LeanFinger finger)
        {
            ...
            {
                TrackableHit hit;
                if (ARCoreWorldOriginHelper.Raycast(finger.ScreenPosition.x, finger.ScreenPosition.y,
            ...
            {
                Pose hitPose;
                if (m_ARKit.RaycastPlane(ARKitFirstPersonCamera, finger.ScreenPosition.x, finger.ScreenPosition.y, out hitPose))
                {
                    m_LastPlacedAnchor = m_ARKit.CreateAnchor(hitPose);
                }
            }

            //If an OBject was in the scene

            if (numberOfObjects < numberOfObjectsAllowed)
            {
        ...
        void ChangeObjToCube() { }

        public void ChangeToSushi()
        {
            GameObject.Find("LocalPlayer").GetComponent<LocalPlayerController>().CmdDestroyUdon();
            if (numberOfObjects == 1)
            {
                numberOfObjects = numberOfObjects - 1;
                GameObject.Find("LocalPlayer").GetComponent<LocalPlayerController>().CmdDestroyUdon();
                GameObject.Find("LocalPlayer").GetComponent<LocalPlayerController>().CmdDestroySushi();
                GameObject.Find("LocalPlayer").GetComponent<LocalPlayerController>().CmdDestroyHotdog();
                GameObject.Find("LocalPlayer").GetComponent<LocalPlayerController>().CmdDestroyApplePie();
            }
        ...
        private void _InstantiateStar()
        {
            if (choice == 0) {
                ...
                //GameObject.Find("LocalPlayer").GetComponent<LocalPlayerController>().CmdSpawnStar(m_LastPlacedAnchor.transform.position, m_LastPlacedAnchor.transform.rotation);
                //numberOfObjects = numberOfObjects + 1;
            }
            else if (choice == 1) {
                // Star must be spawned in the server so a networking Command is used.
                GameObject.Find("LocalPlayer").GetComponent<LocalPlayerController>()
                          .CmdSpawnUdon(m_LastPlacedAnchor.transform.position, m_LastPlacedAnchor.transform.rotation);
                numberOfObjects = numberOfObjects + 1;
            }
            else if (choice == 2) {
                // Star must be spawned in the server so a networking Command is used.
                GameObject.Find("LocalPlayer").GetComponent<LocalPlayerController>()
                          .CmdSpawnSushi(m_LastPlacedAnchor.transform.position, m_LastPlacedAnchor.transform.rotation);
                numberOfObjects = numberOfObjects + 1;
            }
            else if (choice == 3)
            {
                // Star must be spawned in the server so a networking Command is used.
                GameObject.Find("LocalPlayer").GetComponent<LocalPlayerController>()
                          .CmdSpawnHotdog(m_LastPlacedAnchor.transform.position, m_LastPlacedAnchor.transform.rotation);
                numberOfObjects = numberOfObjects + 1;
            }
            else if (choice == 4)
            {
                // Star must be spawned in the server so a networking Command is used.
                GameObject.Find("LocalPlayer").GetComponent<LocalPlayerController>()
                          .CmdSpawnApplePie(m_LastPlacedAnchor.transform.position, m_LastPlacedAnchor.transform.rotation);
                numberOfObjects = numberOfObjects + 1;
            }
